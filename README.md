...
| `/select_server <password> <number>`     | Включает указанный сервер (по номеру в списке) — **только для администраторов** |
...
| `/server_command <password> <command>`   | Выполняет команду на выбранном сервере — **только для администраторов**         |
